(function () {
    var webApiService = function ($http) {

        var getToken = function () {
            var userName = 'locafox';
            var password = 'LocaF#xes!';
            var encoded = Base64.encode(userName + ':' + password);
            $http.defaults.headers.common.Authorization = 'Basic ' + encoded;
            var url = "https://foxtest.herokuapp.com/v1/token";

            return $http.post(url).then(onSuccess, onError);
        }
        ...
        var getMapLocations = function () {
            return getToken().then(function (response) {
                if (response.success) {
                    ...
                    var url = "https://foxtest.herokuapp.com/v1/offers?token=" + token;
