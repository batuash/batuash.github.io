...
function initHeaderTitle() {
  $('.mainHeader').html(
    convertMonthNumberToString(month) + ' ' + day + ', ' + year
  );
}
...
function initDatalist() {
  ...
  for (var i = 1; i <= currentDayDate; i++) {
    datalist[i] = new ItemObject(
      'item' + i,
      localStorage.getItem(getItemEnterKey(i)),
      localStorage.getItem(getItemLeaveKey(i))
    );
  }
}
...
function refreshTable() {
  ...
  dayesToWork = 0;
  for (
    var counter = firsttDayOfTheMonth - 1,
      i = 1,
      dayOfMonth = firsttDayOfTheMonth;
    counter < currentDayDate + firsttDayOfTheMonth - 1;
    ...
    if (counter > firsttDayOfTheMonth - 2 && !isWeekendDay(dayOfMonth)) {
      ...
      $('td:eq(' + counter + ')').html(
        '<div class="table-item-top">' +
          todayDate +
          '</div><div class="table-item-bottom"><span class="enterTime">' +
          datalist[i].enter +
...
function updateInfoData() {
  ...
  else houresExtra = parseFloat((houresWorked - houresToWork).toFixed(2));
}
